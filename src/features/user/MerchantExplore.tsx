// src/features/merchant/MerchantExplore.tsx
...
const MerchantExplore: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  }, [merchants, searchZip, selectedCategory]);
      ...
      <main className="max-w-2xl mx-auto px-6">
        ...
        <div className="bg-white p-8 rounded-[40px] shadow-2xl border-4 border-[#0f172a] mb-12 -mt-20 relative z-20">
          ...
          {(searchZip || selectedCategory !== 'all') && (
            ...
            >
              <XCircle size={14} /> Resetar Pesquisa
            </button>
          )}
        </div>
            ...
            {filteredMerchants.length === 0 ? (
              <div className="bg-white p-20 rounded-[60px] border-4 border-dashed border-slate-200 text-center">
                ...
                <p className="text-slate-400 font-black text-xs uppercase italic tracking-widest">
                  Nenhum parceiro encontrado nesta zona.
                </p>
              </div>
            ) : (
              filteredMerchants.map(m => (
                <div key={m.id} className="bg-white rounded-[50px] shadow-xl border-2 border-slate-100 overflow-hidden group hover:border-[#00d66f] transition-all duration-500">
                  <div className="flex flex-col sm:flex-row">
                    {/* INFO LATERIAL / MOBILE TOP */}
                    <div className="bg-[#0f172a] p-8 sm:w-48 flex flex-col items-center justify-center gap-2 shrink-0">
                      <div className="bg-[#00d66f] w-full py-4 rounded-3xl shadow-lg shadow-[#00d66f]/20 transform group-hover:scale-110 transition-transform text-center">
                    ...
                    <div className="p-8 flex-grow">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <span className="inline-block bg-slate-100 text-[#0f172a] px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-3">
                            {m.category || 'Comércio Local'}
                          </span>
                          <h3 className="font-black text-[#0f172a] text-2xl uppercase tracking-tighter leading-none group-hover:text-[#00d66f] transition-colors">
                            {m.name}
                          </h3>
                        </div>
                      ...
                      <div className="pt-6 border-t-2 border-slate-50 grid grid-cols-1 gap-4">
