  ...
  .onUpdate(async (change, context) => {
    ...
    if (before && after && before.status !== 'cancelled' && after.status === 'cancelled') {
      ...
      try {
        ...
        if (!storeWallets[after.merchantId]) return;
        ...
        await batch.commit();
      } catch (error: any) { console.error("Erro na reversão:", error); }
    }
  });
...
export const onNewFeedback = functions.region("us-central1").firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    } catch (error) { console.error("Erro ao notificar feedback:", error); }
  });
  ...
  .onWrite(async (change, context) => {
    ...
    try {
      ...
      usersSnap.forEach(doc => {
        ...
        else if (notif.targetType === 'email') { if (user.email === notif.targetValue?.toLowerCase()) matches = true; }
        else if (notif.targetType === 'birthDate') { if (user.birthDate && user.birthDate.split('-')[1] === notif.targetValue) matches = true; }
        else if (notif.targetType === 'zonas') {
          const zones = notif.targetZones || [];
          if (zones.some((z: string) => z.includes(`Freguesia: ${user.freguesia}`) || z.includes(`Concelho: ${user.concelho}`) || z.includes(`Distrito: ${user.distrito}`))) matches = true;
        }
    ...
    } catch (error) { console.error("Erro fatal no disparo de Push Admin:", error); }
  });
    ...
    if (before && after && before.status !== 'approved' && after.status === 'approved' && after.type === 'push_notification' && after.sent !== true) {
      try {
        ...
        usersSnap.forEach(doc => {
          ...
          else if (after.targetType === 'birthDate') { const cm = new Date().getMonth() + 1; if (user.birthDate && parseInt(user.birthDate.split('-')[1]) === cm) matches = true; }
      ...
      } catch (error) { console.error("Erro no Push do Lojista:", error); }
    }
  });
  ...
  .onWrite(async (change, context) => {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (!change.before.exists && change.after.exists) currentCount += 1;
        else if (change.before.exists && !change.after.exists) currentCount = Math.max(0, currentCount - 1);
        transaction.set(statsRef, { membersCount: currentCount, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
      });
    } catch (error) { console.error("Erro ao atualizar contador de membros:", error); }
  });

/**
 * 8. GERAÇÃO DE CACHE CENTRALIZADO (Agendado diariamente)
 */
export const generateAppCache = functions.region("us-central1").pubsub.schedule("every 24 hours").onRun(async (context: functions.EventContext) => {
  try {
    const cache: any = {
    ...
    if (locDoc.exists) cache.locations = locDoc.data()?.data || {};
    const statsDoc = await db.collection("system").doc("stats").get();
    if (statsDoc.exists) cache.stats = statsDoc.data() || { membersCount: 0 };
    const configDoc = await db.collection("system").doc("config").get();
    if (configDoc.exists) cache.config = configDoc.data() || {};
    const pricingSnap = await db.collection("pricing_rules").get();
    pricingSnap.forEach(doc => cache.pricingRules.push({ id: doc.id, ...doc.data() }));
    const faqsSnap = await db.collection("municipalities_faqs").orderBy("createdAt", "desc").limit(100).get();
    faqsSnap.forEach(doc => cache.municipalitiesFaqs.push({ id: doc.id, ...doc.data() }));
    const campaignsSnap = await db.collection("leaflet_campaigns").where("isActive", "==", true).get();
    campaignsSnap.forEach(doc => cache.leafletCampaigns.push({ id: doc.id, ...doc.data() }));
    await db.collection("system").doc("app_cache").set(cache);
    console.log("Cache gerado com sucesso.");
  } catch (error) { console.error("Erro ao gerar app_cache:", error); }
  return null;
});
 ...
 */
export const cleanupExpiredData = functions.region("us-central1").pubsub.schedule("every 24 hours").onRun(async (context: functions.EventContext) => {
  const batchSize = 100;
 ...
 */
export const refreshAppCache = functions.region("us-central1").https.onCall(async (data, context) => {
