...
const MerchantTerminal: React.FC<MerchantTerminalProps> = ({
  ...
  // Formatar o input do NIF para agrupar 3 em 3 (Visual apenas)
  const handleNifChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let val = e.target.value.replace(/\D/g, ''); // Remove tudo o que não for número
    if (val.length > 9) val = val.substring(0, 9);
    
    // Formatação visual: 123 456 789
    let formatted = val;
    if (val.length > 6) {
  ...
  // Validar se o botão de Ganhar/Descontar deve estar ativo
  const canProcess = !isLoading && foundClient !== null && Number(amount) > 0 && documentNumber.trim().length > 0;
  // Validar se pode cancelar (Apenas precisa do NIF e Doc)
  const canCancel = !isLoading && foundClient !== null && documentNumber.trim().length > 0;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
      
      {/* LADO ESQUERDO: TERMINAL DE INTRODUÇÃO */}
      <div className="lg:col-span-2 bg-white p-8 md:p-12 rounded-[40px] shadow-[12px_12px_0px_0px_#0a2540] border-4 border-[#0a2540] relative overflow-hidden">
        <div className="space-y-10 relative z-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                className="bg-[#0a2540] px-8 rounded-3xl text-[#00d66f] hover:bg-black transition-all shadow-[6px_6px_0px_0px_#00d66f] border-2 border-[#00d66f] active:translate-y-1 active:shadow-none"
                title="Ler QR Code"
              >
                <Camera size={32} />
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* VALOR DA FATURA */}
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">Valor Fatura (€)</label>
              <input 
                type="number" 
                step="0.01"
                min="0"
              ...
              {Number(amount) > 0 && (
                <div className="flex items-center gap-3 bg-green-50 p-4 rounded-2xl border-2 border-green-200 animate-in zoom-in">
                  <ArrowRight size={16} className="text-[#00d66f]" />
                  <span className="text-[11px] font-black uppercase text-[#0a2540] tracking-widest">
                    Cashback Emitido: <span className="text-[#00d66f] text-sm italic ml-1">{formatCurrency(previewCashback)}</span>
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <input 
                ...
                className="w-full p-6 bg-slate-50 border-4 border-slate-100 rounded-3xl text-2xl font-black uppercase text-[#0a2540] outline-none focus:border-[#0a2540] shadow-inner tracking-widest" 
              />
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest ml-2 mt-2 leading-relaxed">
                * Obrigatório para auditorias e cruzamento de dados com a Autoridade Tributária em caso de disputa.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* LADO DIREITO: AÇÕES DO LOJISTA */}
      <div className="flex flex-col gap-4">
        
        {/* CAIXA DE IDENTIFICAÇÃO (STATUS) */}
        <div className={`p-6 rounded-[35px] border-4 transition-all flex flex-col items-center gap-3 justify-center shadow-lg ${foundClient ? 'bg-[#00d66f] border-[#0a2540] text-[#0a2540]' : 'bg-white border-slate-200 text-slate-300 border-dashed'}`}>
          {foundClient ? (
            <>
              <div className="p-3 bg-white/30 rounded-2xl mb-1 border-2 border-[#0a2540]/10">
                <UserIcon size={24} className="text-[#0a2540]" strokeWidth={3} />
              </div>
            ...
            <>
               ...
               <p className="text-[10px] font-black uppercase tracking-widest text-center">{isSearching ? 'A pesquisar base de dados...' : 'Aguardando Cliente...'}</p>
            </>
          )}
        </div>
        ...
        {/* BOTÃO GANHAR (EARN) */}
        <button 
          onClick={() => onProcessAction('earn')} 
          disabled={!canProcess} 
          className="flex-1 bg-[#0a2540] p-6 rounded-[35px] text-white transition-all hover:bg-black hover:scale-[1.02] shadow-[8px_8px_0px_0px_#00d66f] border-2 border-[#0a2540] flex flex-col items-center justify-center gap-3 disabled:opacity-40 disabled:shadow-none disabled:hover:scale-100"
        >
          <Coins size={32} className="text-[#00d66f]" />
          <span className="font-black text-[11px] uppercase tracking-[0.2em]">Atribuir Cashback</span>
        </button>

        {/* BOTÃO UTILIZAR SALDO (REDEEM) */}
        <button 
          onClick={() => onProcessAction('redeem')} 
          disabled={!canProcess || clientStoreBalance <= 0 || Number(amount) > clientStoreBalance} 
          className="flex-1 bg-white p-6 rounded-[35px] text-[#0a2540] border-4 border-[#0a2540] transition-all hover:bg-slate-50 hover:scale-[1.02] shadow-xl flex flex-col items-center justify-center gap-3 disabled:opacity-40 disabled:hover:scale-100"
        >
        ...
        {/* BOTÃO ANULAR */}
        <button 
          onClick={() => onProcessAction('cancel')} 
          disabled={!canCancel} 
          className="py-6 px-4 bg-red-50 rounded-[35px] text-red-500 border-4 border-red-100 transition-all hover:bg-red-500 hover:text-white hover:border-red-600 flex items-center justify-center gap-3 disabled:opacity-40"
        >
