...
import { Search, Users, Mail, Locate, Download, Phone, BellRing, BellOff, Trash2 } from 'lucide-react';
...
const AdminUsers: React.FC<AdminUsersProps> = ({ users, transactions }) => {
  ...
  const handleDeleteUser = async (id: string, role: string) => {
    if (!window.confirm("Atenção: Esta ação é irreversível e vai apagar todo o histórico deste Cliente. Continuar?")) return;
    setIsDeletingId(id);
    try {
      await deleteUserWithHistory(id, role as any);
      toast.success("Cliente e dados eliminados.");
    } catch(err) {
  ...
  const exportToExcel = () => {
    const dataToExport = filteredUsers.map(u => {
      ...
      return {
        ...
        "Equipamentos Ativos": u.devices ? u.devices.length : 0,
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative group flex-1">
          ...
          <input type="text" placeholder="PESQUISAR POR NOME, NIF, EMAIL OU C. POSTAL..." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredUsers.length > 0 ? filteredUsers.map((u) => {
          const devicesCount = u.devices ? u.devices.length : 0;
          const hasNotifications = devicesCount > 0;
          ...
          return (
            <div key={u.id} className="bg-white border-2 border-[#0a2540] rounded-[40px] p-8 shadow-[8px_8px_0px_0px_#0a2540] flex flex-col relative group hover:-translate-y-1 transition-transform">
              ...
              <div className="absolute top-4 right-4">
                {hasNotifications ? (
                  <div className="flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-green-200">
                    <BellRing size={12} /> {devicesCount} Equipamento{devicesCount > 1 ? 's' : ''}
                  </div>
                ) : (
                  <div className="flex items-center gap-1 bg-slate-100 text-slate-500 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-slate-200">
                    <BellOff size={12} /> Sem Alertas
                  </div>
                )}
              </div>
              ...
              <div className="mt-auto flex flex-col gap-2">
                {u.status === 'active' ? (
                  <button onClick={() => updateStatus(u.id, 'disabled')} className="w-full bg-slate-100 hover:bg-orange-50 hover:text-orange-500 text-slate-400 p-4 rounded-2xl font-black uppercase text-[10px] transition-colors">Suspender Conta</button>
                ) : (
                ...
                <button 
                  ...
                  className="w-full bg-red-50 hover:bg-red-500 hover:text-white text-red-500 p-4 rounded-2xl font-black uppercase text-[10px] transition-colors flex items-center justify-center gap-2"
                >
                  {isDeletingId === u.id ? 'A eliminar...' : <><Trash2 size={14} /> Eliminar Cliente</>}
                </button>
              </div>
            </div>
          )
