...
import { 
  ...
  BellRing, BellOff, Trash2, RefreshCw, AlertCircle 
} from 'lucide-react';
...
const AdminUsers: React.FC<AdminUsersProps> = ({ users, transactions }) => {
  ...
  // PONTO 2: Eliminar cliente e histórico pelo Admin
  const handleDeleteUser = async (id: string, name: string) => {
    if (!window.confirm(`ALERTA: Deseja eliminar permanentemente o vizinho "${name}" e todos os seus dados? Esta ação não pode ser revertida.`)) return;
  ...
  const exportToExcel = () => {
    const dataToExport = filteredUsers.map(u => {
      ...
      return {
        ...
        Telefone: u.phone || '---',
        NIF: u.nif || '---',
        "Equipamentos Ativos": u.devices ? u.devices.length : 0, // PONTO 3
        ...
        {filteredUsers.length > 0 ? filteredUsers.map((u) => {
          ...
          return (
            <div key={u.id} className="bg-white border-2 border-[#0a2540] rounded-[40px] p-8 shadow-[8px_8px_0px_0px_#0a2540] flex flex-col relative group hover:-translate-y-1 transition-transform">
                ...
                {/* INDICADOR DE EQUIPAMENTOS (PONTO 3) */}
                ...
                <div className="mt-auto flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button 
                      ...
                      className="w-full bg-red-50 hover:bg-red-500 hover:text-white text-red-500 p-4 rounded-2xl font-black uppercase text-[10px] transition-all flex items-center justify-center gap-2"
                    >
