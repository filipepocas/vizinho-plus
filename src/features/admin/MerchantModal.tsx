...
import { X, Store, Mail, Hash, Percent, MapPin, Loader2, Locate, Tag, User, Phone } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', userCredential.user.uid), {
        ...
        name: formData.name.trim(), // Shop Name
        shopName: formData.name.trim(), // Shop Name redundante para garantir
        responsibleName: formData.responsibleName.trim(), // NOVO CAMPO
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input required type="password" placeholder="••••••••" className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-[#00d66f] outline-none font-bold text-sm" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
            </div>
            <div>
          ...
          <button disabled={loading} type="submit" className="w-full bg-[#00d66f] text-[#0a2540] p-5 rounded-2xl font-black uppercase tracking-widest hover:scale-[1.02] transition-all flex items-center justify-center gap-3 border-b-4 border-[#0a2540] mt-4">
